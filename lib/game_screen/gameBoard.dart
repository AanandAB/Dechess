...
import 'package:knightly/components/square.dart';
...
// If no piece is selected, this is null.
  ...
  // Add the squareSize variable to hold the size of each square on the chessboard.
...
// Default value -1 indicated no piece is currently selected;
  int selectedCol = -1;
  // Move log list to store the moves made during the game
  List<String> moveLog = [];
  // Create a ScrollController to manage the scrolling of the move log
  final ScrollController _moveLogScrollController = ScrollController();

  // Function to update the move log
  void updateMoveLog(String move) {
    setState(() {
      moveLog.add(move);
    });
    WidgetsBinding.instance!.addPostFrameCallback((_) {
      // Scroll to the end of the move log after adding a new move
      _moveLogScrollController.animateTo(
        _moveLogScrollController.position.maxScrollExtent,
...
// add the captured piece to the appropriate list
    ...
    String rowToChessCoordinate(int row) {
      // In chess, rows are represented by numbers 1 to 8, so we add 1 to the row index.
      return (row + 1).toString();
    }

    String colToChessCoordinate(int col) {
      // In chess, columns are represented by letters 'a' to 'h', so we add 'a' ASCII value (97) to the column index.
      return String.fromCharCode('a'.codeUnitAt(0) + col);
    }

    // Append the move to the move log
    String move =
        "${selectedPiece!.isWhite ? 'White' : 'Black'}: ${selectedPiece!.type} ${rowToChessCoordinate(selectedRow)}${colToChessCoordinate(selectedCol)} -> ${rowToChessCoordinate(newRow)}${colToChessCoordinate(newCol)}";
    updateMoveLog(move);
  ...
  Widget build(BuildContext context) {
    // Calculate the squareSize based on the screen width (assuming a square chessboard).
    squareSize = MediaQuery.of(context).size.width / 8;
    return Scaffold(
      backgroundColor: Colors.white30,
      body: Column(
        children: [
// Move Log at the top of the screen
          Container(
            height: 100,
            padding: const EdgeInsets.all(8.0),
            child: SingleChildScrollView(
              controller:
                  _moveLogScrollController, // Attach the ScrollController here
              scrollDirection: Axis.horizontal,
              child: Row(
                children: [
                  for (int index = 0; index < moveLog.length; index++)
                    Padding(
          ...
          Expanded(
            ...
            child: GridView.builder(
              ...
              gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                crossAxisCount: 8,
                childAspectRatio:
                    1.0, // Set the aspect ratio to make squares square-shaped
              ),
              itemBuilder: (context, index) {
                ...
                return Square(
                  size:
                      squareSize, // Pass the calculated square size to the Square widget
